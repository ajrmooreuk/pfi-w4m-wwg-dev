...
## PPM Integration (v1.1.0 — Epic 49, F49.3)

When a PPM project plan is available (`09-ppm-project-plan-{instance}.jsonld`), the EFS skill enriches its output with PPM traceability:

### Optional PPM Input
- `{working_dir}/app-planner-output/09-ppm-project-plan-{instance}.jsonld` — generated by `pfc-ppm-plan`

### PPM-Enriched Output
When PPM input is present, each generated epic carries:
- `efs:targetRelease` populated from PPM milestone dates (via JP-EFS-PPM-001)
- `efs:deliversForProject` reference to the PPM Project @id
- Each feature carries `efs:decomposedByWBS` reference (via JP-EFS-PPM-003)

### PPM Join Patterns

| Pattern | From → To | Rule |
|---------|-----------|------|
| JP-EFS-PPM-001 | ppm:Project → efs:Epic | Project `deliversEpic` populates epic `targetRelease` from milestone due dates |
| JP-EFS-PPM-002 | ppm:Programme → efs:EpicGroup | Programme scope groups related epics |
| JP-EFS-PPM-003 | ppm:WorkBreakdownStructure → efs:Feature | WBS `decomposesFeature` links work packages to features |

### PPM Quality Gate (G6 — optional, only when PPM input exists)
- [ ] Every epic linked to a PPM project via `deliversEpic`
- [ ] Every feature linked to a WBS work package via `decomposesFeature`
- [ ] PPM milestone dates propagated to `targetRelease` fields
...
| `JP-EFS-PPM-001` | Project→Epic delivery traceability (v1.1.0) |
| `JP-EFS-PPM-002` | Programme→EpicGroup scope grouping (v1.1.0) |
| `JP-EFS-PPM-003` | WBS→Feature work decomposition (v1.1.0) |
